...
K4_to_S4_list = [0,2,12,14]
K4_to_S4_dict = {}
S4_to_K4_dict = {}
for i,j in enumerate(K4_to_S4_list):
    K4_to_S4_dict[i] = j 
    S4_to_K4_dict[j] = i 
...
def rot_mat(xx,i):
    return S4_group.rot_mat(xx,K4_to_S4_dict[i]) 


def get_inverse_cayley():
    ict_S4 = S4_group.get_inverse_cayley()
    ict_K4 = np.zeros((len(K4_to_S4_list),len(K4_to_S4_list)),dtype = int)
    
    for i in range(len(K4_to_S4_list)):
        for j in range(len(K4_to_S4_list)):
            s4_i = K4_to_S4_dict[i]
            s4_j = K4_to_S4_dict[j]
            s4_val = ict_S4[s4_i,s4_j]
            k4_val = S4_to_K4_dict[s4_val]
            ict_K4[i,j] = np.int(k4_val)
    return ict_K4
...
class H:
    ...
    rots = [i for i in range(len(K4_to_S4_list))]
