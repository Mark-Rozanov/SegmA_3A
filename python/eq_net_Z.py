...
import Zero_group
layers = layer3D(Zero_group) # The layers is instantiated with the group structure as input
...
class Net(nn.Module):
    def __init__(self, net_name):
        ...
        self.conv1g = layers.ConvRnG(1,50,3, padding = 0).float()
        self.bn1  = BN_group(50)
        self.elu1 = nn.LeakyReLU()
        
        self.conv2g = layers.ConvGG(50,50,3, padding = 0).float()
        self.bn2  = BN_group(50)
        self.elu2 = nn.LeakyReLU()

        self.conv3g = layers.ConvGG(50,64,2, padding = 0).float()
        self.bn3  = BN_group(64)
        self.elu3 = nn.LeakyReLU()

        self.conv4g = layers.ConvGG(64,40,2, padding = 0).float()
        self.bn4  = BN_group(40)
        self.elu4 = nn.LeakyReLU()

        self.conv5g = layers.ConvGG(40,30,3, padding = 0).float()
        self.bn5  = BN_group(30)
        self.elu5 = nn.LeakyReLU()


        self.conv6g = layers.ConvGG(30,22,3, padding = 0).float()
